...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    public int getItemViewType(int position) {
        if (position == 0 && mData.get(position) != null
                && TextUtils.equals("定", mData.get(position).getSection().substring(0, 1)))
            return VIEW_TYPE_LOCATION;
        if (position == 1 && mData.get(position) != null
                && TextUtils.equals("热", mData.get(position).getSection().substring(0, 1)))
