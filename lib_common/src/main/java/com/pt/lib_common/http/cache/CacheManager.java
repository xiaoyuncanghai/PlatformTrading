...
public final class CacheManager {
    ...
    private CacheManager() {
        File diskCacheDir = getDiskCacheDir(Mango.getInstance().context, CACHE_DIR);
        if (!diskCacheDir.exists()) {
            boolean b = diskCacheDir.mkdirs();
            LogUtil.d( "!diskCacheDir.exists() --- diskCacheDir.mkdirs()=" + b);
        }
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(Mango.getInstance().context), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
                LogUtil.d( "mDiskLruCache created");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
